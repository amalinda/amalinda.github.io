...
static_extensions = [
		...
		'.h', '.c', '.cpp', '.cxx', '.mk', '.ipynb', '.py','.asc','.opencert',
		'.zip', '.webm', '.patch', '.mp4', '.html'
		]
image_extensions = [
		'.png', '.jpg',
		]
...
if __name__ == '__main__':
	...
	#DirectoryWalker().process('classes') #uncomment later to compile classes folder. Amalinda

	logger.info('Building publications database...')
	#publications.generate_publications_page(pubs_groups, jinja_env) #uncomment later to compile publications.html Amalinda

	# Put all static content in the html folder
	logger.info('Copying static content...')
	for dirpath,dirnames,filenames in os.walk('static'): #now compiling static flder.
